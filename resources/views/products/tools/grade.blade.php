        ...
        <div class="form-horizontal">
          <div class="form-group">
              ...
              <div class="inner-addon left-addon col-xs-2 col-sm-2">
                <button type="button" class="btn btn-inbloom" data-toggle="modal" data-target="#myRegister" onclick="setRegistros('','','setInsertGrade')">Add</button>
              </div>
            </div>
        </div>
    </article>
          ...
          <tbody>
            <tr>
              @foreach ($tblDatos as $datos)
                <tr>
                  <td>{{$datos->ID_GRADE}}</td> <td>{{$datos->NAME_GRADE}}</td> <td>{{$datos->DATE_GRADE}}</td>
                  <td>
                    <div class="btn-group">
