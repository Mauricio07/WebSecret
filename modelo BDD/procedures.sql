...
ELSE
	BEGIN
		...
		SELECT @idRecipeUpdate as'ID_RECIPE' FROM RECIPES
	END
go
...
BEGIN TRANSACTION
	BEGIN TRY
		...
		begin
			...
			SELECT @id_ProdUpdate AS'ID' FROM PRODUCTS;
		end
		COMMIT --add
	END TRY
	BEGIN CATCH
...
begin	
	...
	EXEC SP_HABILITA_PRODUCTO @IdProd;
end
...
begin
	set @id_Receta=(select ID_RECIPE from PRODUCT_RECIPIES where ID_PRODUCT=@IdProd);
	-- receta items
...
AS
SELECT RECIPE_ITEMS.ID_RECIPE, MATERIALS_ITEMS.NAME_MATERIALS, RECIPE_ITEMS.QUANTITY_RECIPEMAT
