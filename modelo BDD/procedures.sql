...
AS
SELECT B.ID_BOX, B.NAME_BOX, BT.TYPEBOXE_BTYPE, B.ACRONYM_BOX, CONCAT(B.HEIGHT_BOX ,' X ', B.WIDTH_BOX, ' X ',B.LENGTH_BOX)AS 'DIMENSSION', WB.KG_WEIGHT
...
CREATE PROCEDURE SP_ADD_TYPEBOXES @type int, @name varchar(80), @acronym varchar(10), @length float, @width float, @height float, @weight float
AS
-- Insert Wight boxes
...
-- insrt Boxes
INSERT INTO BOXES(ID_BTYPE, ID_WEIGHT, NAME_BOX, HEIGHT_BOX, WIDTH_BOX, LENGTH_BOX, DATE_BOX, ACRONYM_BOX)
VALUES(@type, @idWeight, @name, @height, @width ,@length, GETDATE(), @acronym);

GO

CREATE VIEW VW_REP_BOXES 
AS
SELECT BT.TYPEBOXE_BTYPE, B.ID_BOX, B.NAME_BOX, B.ACRONYM_BOX, B.LENGTH_BOX, B.WIDTH_BOX, B.HEIGHT_BOX, W.LB_WEIGHT 
