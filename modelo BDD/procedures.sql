...
SELECT (SELECT NAME_SPECIE FROM SPECIES WHERE ID_SPECIE=@idSpecie)AS'SPECIE',
	   ...
	   (SELECT NAME_GRADE FROM GRADES WHERE ID_GRADE=@idGrade)AS'GRADE',@quantity AS 'QUANTITY', @indexItemRecipe as 'INDEXITEMRECIPE'
...
go
ALTER PROCEDURE ASP_MATERIALS_PRODUCTS @idProduct int
AS
select ROW_NUMBER() OVER(ORDER BY P.ID_PRODUCT)as'INDEXMATPROD', P.ID_PRODUCT, MI.ID_MATERIAL, MI.NAME_MATERIALS, MAPR.QUANTITY_PRODMAT 
...
ALTER PROCEDURE ASP_RECIPE_PRODUCTS @idProduct int
AS
select P.ID_PRODUCT, R.ID_RECIPE, PRE.NAME_PTYPE,PR.PACK
from PRODUCT_RECIPIES PR, RECIPES R,PRODUCTS P, PRESENTATIONES PRE
WHERE PR.ID_PRODUCT=P.ID_PRODUCT
AND PR.ID_RECIPE=R.ID_RECIPE
AND R.ID_PTYPE=PRE.ID_PTYPE
AND P.ID_PRODUCT=@idProduct
AND R.STATUS_RECIPE=1
AND P.ID_PRODUCT=@idProduct

GO
CREATE PROCEDURE ASP_RECIPES_ITEMS @idRecipe int
AS
...
FROM RECIPES, ITEMS, ITEMS_RECIPES, COLORS,PROCESS, SPECIES, GRADES, CUTS, VARIETIES, ITEMS_TYPES
WHERE RECIPES.ID_RECIPE=ITEMS_RECIPES.ID_RECIPE
...
AS
SELECT ITEMS.ID_ITEM, RECIPE_ITEMS.ID_RECIPE, MATERIALS_ITEMS.NAME_MATERIALS, RECIPE_ITEMS.QUANTITY_RECIPEMAT
